...
def pretrain_encoder(args, mixer, optimM):
    for enc_iter in range(1000):
        ...
        codes = mixer(s).reshape(args.z*args.ngen, 1000).transpose(0, 1)
...
def calc_d_loss(args, Dz, z, codes, ngen):
    log_pz = log_density(z, 2).reshape(-1, 1).to(args.device)
    dim = args.k * ngen
    zeros = torch.zeros(dim, 1, requires_grad=True).to(args.device)
    ones = torch.ones(args.k, 1, requires_grad=True).to(args.device)
    d_z = Dz(z)
    codes = codes.reshape(-1, args.z)
    d_codes = Dz(codes)
    log_pz_ = log_density(torch.ones(dim, 1), 2).reshape(-1, 1).to(args.device)
